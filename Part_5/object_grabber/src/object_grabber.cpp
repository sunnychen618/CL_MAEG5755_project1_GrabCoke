...
int  ObjectGrabber::grab_object(int object_id,geometry_msgs::PoseStamped object_pose_stamped){
    ...
    depart_pose_ = grasp_pose_;
    depart_pose_.pose.position.z += 0.1;
...
int  ObjectGrabber::dropoff_object(int object_id,geometry_msgs::PoseStamped desired_object_pose_stamped){
    ...
    approach_pose_ = depart_pose_;

    if (desired_object_pose_stamped.pose.position.x - grasp_pose_.pose.position.x > 0.05)
    {
        ROS_WARN("DETECT CHANGE ALONG X AXIS POSITIVE DIRECTION");
        approach_pose_.pose.position.x += 0.1 ;
    }
    else if (desired_object_pose_stamped.pose.position.x - grasp_pose_.pose.position.x < -0.05)
    {
        ROS_WARN("DETECT CHANGE ALONG X AXIS NEGATIVE DIRECTION");
        approach_pose_.pose.position.x -= 0.1 ;
    }   
    else if (desired_object_pose_stamped.pose.position.y - grasp_pose_.pose.position.y > 0.05)
    {
        ROS_WARN("DETECT CHANGE ALONG X AXIS NEGATIVE DIRECTION");
        approach_pose_.pose.position.y += 0.1 ;
    }   
    else if (desired_object_pose_stamped.pose.position.y - grasp_pose_.pose.position.y < -0.05)
    {
        ROS_WARN("DETECT CHANGE ALONG X AXIS NEGATIVE DIRECTION");
    ...
    ROS_WARN("planning motion of gripper to approach pose at: ");
    xformUtils.printPose(approach_pose_);
    ...
    // tf::StampedTransform pre_place_pose = xformUtils.convert_poseStamped_to_stampedTransform(desired_object_pose_stamped, "object_frame");
    // dropoff_pose_ = xformUtils.get_pose_from_stamped_tf(pre_place_pose);  
    // ROS_WARN("planning motion of gripper to dropoff pose at: ");
    // dropoff_pose_.pose.position.x += 0.1 ;
    // dropoff_pose_.pose.orientation.x = 0;
    // dropoff_pose_.pose.orientation.y = 0;
    // dropoff_pose_.pose.orientation.z = 1;
    // dropoff_pose_.pose.orientation.w = 0;
    // xformUtils.printPose(dropoff_pose_);
    
    // dropoff_pose_ = approach_pose_;
    // approach_pose_.pose.position.x -= 0.10;
