...
int main(int argc, char** argv) {
    ...
    goal.pickup_frame = g_object_pose;
    goal.object_code= ObjectIdCodes::COKE_CAN_UPRIGHT;
    action_client.sendGoal(goal, &doneCb, &activeCb, &feedbackCb);
    while (!g_goal_done) {
        ros::Duration(0.1).sleep();
    ...
    dropoff_pose.pose.position.x = g_object_pose.pose.position.x + 0.1;
    dropoff_pose.pose.position.y = g_object_pose.pose.position.y ;
    dropoff_pose.pose.position.z = g_object_pose.pose.position.z;
    dropoff_pose.pose.orientation.x = 0;
    dropoff_pose.pose.orientation.y = 0;
    dropoff_pose.pose.orientation.z = 1;
    dropoff_pose.pose.orientation.w = 0;
    dropoff_pose.header.stamp = ros::Time::now();

    //send command to set grasped block on top of existing coke can:
    ROS_INFO("sending a goal: dropoff coke can");
    g_goal_done = false;
    ...
    if (g_callback_status!= coordinator::ManipTaskResult::MANIP_SUCCESS)
    {
        ROS_ERROR("failed to drop off coke can; quitting");
        return 0;
    }
    ...
    //define a dropoff pose;
    dropoff_pose.header.frame_id = "torso";
    dropoff_pose.pose.position.x = g_object_pose.pose.position.x;
    dropoff_pose.pose.position.y = g_object_pose.pose.position.y + 0.1;
    dropoff_pose.pose.position.z = g_object_pose.pose.position.z;
    dropoff_pose.pose.orientation.x = 0;
    dropoff_pose.pose.orientation.y = 0;
    dropoff_pose.pose.orientation.z = 1;
